...
### 첫 번째 타겟 ({targets[0]['name']})부터 리뷰지를 작성하고, 등록된 모든 타겟({len(targets)}명)에 대해 같은 형식으로 반복.
...
| 타겟 | 등급 | 근거 |
|------|------|------|
(등록된 모든 타겟({len(targets)}명) 각 행 — PASS / CONSIDER / RECOMMEND + 이유 1줄)
...
### 타겟 매칭도 매트릭스
등록된 모든 타겟({len(targets)}명)을 행으로 표 작성:
| 타겟 | 1차반응 | 몰입도 | 구매의향 | 종합 |
|------|---------|-------|---------|-----|
(각 타겟 행으로 — 1~10점)
