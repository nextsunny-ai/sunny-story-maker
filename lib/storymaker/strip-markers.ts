...
export function fixCommonTypos(text: string): string {
  ...
  return text
    ...
    //   ㄹ 첨가 — 「하려」를 「할려」로 쓰는 흔한 비문.
    //   시나리오 지문에 자주 나온다 (실측: "손가락을 손목에 댈려 한다")
    .replace(/([가-힣])ㄹ려\s*(하|한다|했)/g, "$1려 $2")
    .replace(/댈려/g, "대려")
    .replace(/할려/g, "하려")
    .replace(/갈려/g, "가려")
    .replace(/볼려/g, "보려")
    .replace(/올려고/g, "오려고")
    .replace(/줄려/g, "주려")
    .replace(/살려고 한다/g, "살려고 한다")   // 이건 정상 — 위 규칙에 걸리지 않게 그대로
    //   카메라 지시 — 「밀어낸다」는 밖으로 미는 뜻이다. 안으로 들어가는 건 「밀고 들어간다」
    .replace(/카메라가([^\n]{0,12})위로\s*밀어낸다/g, "카메라가$1위로 밀고 들어간다")
    .replace(/카메라가([^\n]{0,12})밀어낸다/g, "카메라가$1밀고 들어간다");
}
