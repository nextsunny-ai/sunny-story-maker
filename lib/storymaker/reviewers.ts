// 리뷰어 페르소나 라이브러리 — V1 modules/reviewers.py 그대로 포팅
// 20명의 정밀 설계된 가상 리뷰어 + 12장르별 추천 preset.
//
// 핵심 원칙:
// - 모든 리뷰어는 공식적·정중한 평가서 톤 (말투 차별 X)
// - 페르소나 차별은 오직 "작품을 보는 시각" — 분석 5축 + 평가서 키워드
...
export function recommendForGenre(genreLetter: string, limit = 4): Reviewer[] {
  const ids =
    GENRE_PRESETS[genreLetter] ??
    ["30F_office_seoul", "30M_office_seoul", "casual_30s", "global_kfan"];
  return ids
    .slice(0, limit)
    .map(getReviewerById)
    .filter((r): r is Reviewer => Boolean(r));
}